// app.js - COMPLETE VERSION WITH GLOBAL FUNCTION EXPOSURE
...
// FIX: Remove duplicate declaration - currentUserId is declared in checkAuth()
// let currentUserId = null; // REMOVE THIS LINE - it's declared elsewhere
...
async function initUserDatabase() {
    ...
    // FIX: Use currentUserId from checkAuth() function scope
    if (!currentUserId) {
    ...
    try {
        ...
        if (typeof window.addDebugLog === 'function') {
            window.addDebugLog(`app.js: Initializing database for user: ${currentUserId}`, 'info');
        }
        
        // FIX: Set the user ID first before initializing database
        ...
        if (typeof window.dbAPI?.initDatabaseForUser === 'function') {
            ...
            console.log("✅ Using fallback database setup for user:", currentUserId);
            if (typeof window.addDebugLog === 'function') {
...
async function checkAuth() {
    ...
    // FIX: currentUserId should be declared here
    let currentUserId = null; // DECLARE HERE - THIS IS THE CORRECT PLACE
    ...
    if (authToken && currentUser && currentUserId) {
        ...
        // FIX: currentUserId is available in this scope
        ...
        // currentUserId is already set in this function scope
...
async function login() {
    ...
    try {
        ...
        if (result.success && result.token && result.user) {
            ...
            // FIX: currentUserId will be set in checkAuth() from localStorage
            // Don't set it here directly, let checkAuth() handle it
...
function logout() {
    ...
    if (confirm("Are you sure you want to logout?")) {
        ...
        authToken = null;
        currentUser = null;
        // FIX: currentUserId is a local variable in checkAuth(), so we don't need to clear it here
...
function clearOtherUserData() {
    ...
    // FIX: We need to get currentUserId from localStorage since it's not in global scope
    const userStr = localStorage.getItem('auth_user');
    if (userStr) {
        try {
            ...
            console.error("Error parsing user data in clearOtherUserData:", e);
        }
    }
}
...
function initUserData() {
    ...
    // FIX: Get currentUserId from localStorage since it's not in global scope
    const userStr = localStorage.getItem('auth_user');
    if (!userStr) {
        console.error("Cannot init user data: No user in localStorage");
        return;
    }
    ...
    try {
        ...
        console.error("Error parsing user data in initUserData:", e);
    }
}

function saveUserHeads() {
    // FIX: Get currentUserId from localStorage
    const userStr = localStorage.getItem('auth_user');
    if (!userStr) {
        console.error("Cannot save heads: No user in localStorage");
        return;
    }
    ...
    try {
        ...
        console.error("Error parsing user data in saveUserHeads:", e);
    }
}
...
function preloadCategoriesFromEntries() {
    ...
    try {
        ...
        console.error("Error parsing user data in preloadCategoriesFromEntries:", e);
        setTimeout(preloadCategoriesFromEntries, 100);
    }
}
...
function generateEntryId() {
    ...
    try {
        ...
        console.error("Error parsing user data in generateEntryId:", e);
        return 'temp_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    }
}
...
function initApp() {
    ...
    // FIX: Get currentUserId from localStorage for logging
    const userStr = localStorage.getItem('auth_user');
    ...
    if (userStr) {
        try {
            ...
            console.error("Error parsing user data in initApp:", e);
        }
    }
...
async function syncFromCloud() {
    ...
    if (userStr) {
        try {
            ...
            console.error("Error parsing user data in syncFromCloud:", e);
        }
    }
    ...
    try {
        ...
        if (data.success && data.expenses && data.expenses.length > 0) {
            ...
            if (typeof window.addDebugLog === 'function') {
                window.addDebugLog(`app.js: Sync FROM Cloud complete: ${importedCount} imported`, 'success');
            }
        ...
        if (typeof window.addDebugLog === 'function') {
            window.addDebugLog(`app.js: Sync FROM Cloud error: ${error.message}`, 'error');
        }
...
async function syncWithCloud() {
    ...
    if (userStr) {
        try {
            ...
            console.error("Error parsing user data in syncWithCloud:", e);
        }
    }
    ...
    try {
        ...
        if (typeof window.addDebugLog === 'function') {
            window.addDebugLog(`app.js: Found ${userUnsyncedEntries.length} unsynced entries`, 'info');
        }
        ...
        if (totalErrors === 0) {
            ...
            if (typeof window.addDebugLog === 'function') {
                window.addDebugLog('app.js: Sync TO Cloud complete successfully', 'success');
            }
        } else {
        ...
        if (typeof window.addDebugLog === 'function') {
            window.addDebugLog(`app.js: Sync TO Cloud error: ${error.message}`, 'error');
        }
