...
async function initUserData() {
    ...
    // Load heads from IndexedDB
    if (typeof window.dbAPI?.getUserHeads === 'function') {
        const userHeads = await window.dbAPI.getUserHeads();
        heads = userHeads || {};
    } else {
    ...
    console.log("✅ User data initialized, heads count:", Object.keys(heads).length);
}
...
async function deleteMainHead(m) {
    if (typeof window.addDebugLog === 'function') {
...
async function deleteSubHead(m, s) {
    if (typeof window.addDebugLog === 'function') {
...
console.log("✅ app.js COMPLETELY LOADED - USER ISOLATION VERSION");

if (typeof window.addDebugLog === 'function') {
    window.addDebugLog('app.js: COMPLETELY LOADED - User isolation version', 'success');
