...
async function getAllPossibleDatabases() {
    ...
    try {
        ...
        const commonSuffixes = ['pUqxbButo4', 'pUqxWta4Kz']; // Add known user IDs
...
function clearOtherUsersFromLocalStorage(currentUserId) {
    ...
    allKeys.forEach(key => {
        // Remove localStorage entries that contain other user IDs
        if (key.startsWith('heads_') || key.includes('_sync') || key.includes('_last')) {
            ...
            // Check for user ID patterns in the key
            const patterns = [
                'pUqxbButo4', // Your first user
                'pUqxWta4Kz'  // Your second user
            ];
...
function renderTemp() {
    try {
        ...
        // Create fixed height container that scrolls internally - EXACTLY like ledger page
        ...
        table.className = "fixed-table";  // Use fixed-table class like ledger
        table.style.minWidth = "800px";
        table.style.width = "100%";
        table.style.borderCollapse = "collapse";
        
        // Create thead with sticky positioning - EXACTLY like ledger page
...
// NEW FUNCTION: Paste CSV Data directly from textarea
function showPasteCSVDialog() {
    // Create modal dialog for pasting CSV data
...
function parseCSVLines(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Expected columns: Date, Description, Main Category, Sub Category, Amount
        if (cleanParts.length >= 5) {
            try {
                let amount = parseFloat(cleanParts[4]);
                // If amount is positive but user meant expense (e.g., with - sign), check for minus
                if (!isNaN(amount)) {
                    const entry = {
                        date: cleanParts[0],
                        desc: cleanParts[1],
...
async function processImportRows(rowsToImport) {
    ...
    for (let i = 0; i < rowsToImport.length; i++) {
        try {
            ...
            // Track categories for later addition
            if (entry.main) {
    ...
    // Save categories if any new ones were added
...
function setupEntryScroll() {
    // Let CSS handle scrolling, just ensure containers exist
    const tempList = document.getElementById("tempList");
    ...
    getUserEntries(d => {
        try {
            ...
            filtered.forEach((e, i) => {
                ...
                // Updated column order: Date | Main Category | Sub Category | Description | Amount | Actions
                row.innerHTML = `
                    <td>${e.date}</td>
                    <td>${e.main}</td>
                    <td>${e.sub}</td>
