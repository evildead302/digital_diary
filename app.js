// app.js - COMPLETE ORIGINAL WITH ALL FIXES APPLIED (2591+ lines)
...
class Base61 {
  ...
  // FIXED: Generate entry ID with timestamp + random + user ID (user ID stays as-is, NOT encoded)
...
function preloadCategoriesFromEntries() {
  ...
  getAllEntries(userEntries => {
    const userSpecificEntries = userEntries.filter(entry => 
      entry.userId === currentUserId || !entry.userId
    );
    
    // Extract categories from entries
    userSpecificEntries.forEach(entry => {
...
// Batch generate IDs for CSV import
let csvImportQueue = [];
let csvImportInterval = null;

function startCSVImportIdGeneration(count) {
  return new Promise((resolve) => {
    const ids = [];
    let generated = 0;
    
    const generateBatch = () => {
      for (let i = 0; i < 10 && generated < count; i++) {
    ...
    // Generate remaining in batches every 200ms
    if (generated < count) {
      csvImportInterval = setInterval(generateBatch, 200);
    }
  });
}
  ...
  try {
    ...
    tempEntries.forEach((entry, index) => {
      ...
      row.innerHTML = `
        ...
        <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px; text-align: right; font-weight: 700; font-size: 1.05em;" class="${entry.amount < 0 ? 'amount-negative' : 'amount-positive'}">
...
async function syncFromCloud() {
  ...
  try {
    ...
    if (data.success && data.expenses && data.expenses.length > 0) {
      ...
      // Get user-specific local entries
      const userLocalEntries = await new Promise(resolve => {
      ...
      userLocalEntries.forEach(entry => {
      ...
      console.log(`Sync from cloud complete: ${importedCount} imported, ${updatedCount} updated`);
...
async function syncWithCloud() {
  ...
  try {
    ...
    const userUnsyncedEntries = await new Promise(resolve => {
      getAllEntries(entries => {
    ...
    console.log(`Syncing ${userUnsyncedEntries.length} entries to cloud...`);
      ...
      for (const deletedEntry of deletedEntries) {
        try {
          ...
          if (!deleteResponse.ok) {
            ...
            await deleteEntryPermanently(deletedEntry.id);
            deletedSuccessCount++;
          }
        } catch (deleteError) {
...
// Updated markEntryAsDeleted function
function markEntryAsDeleted(id) {
  if (!confirm("Permanently delete this entry? This cannot be undone.")) {
    return;
  }
  
  // For local only (will be permanently deleted on next sync)
  deleteEntryPermanently(id).then(() => {
    loadSaved();
    calcTotal();
    loadLedger();
    alert("✅ Entry permanently deleted from local storage.");
  }).catch(error => {
...
async function loadDataFromCloudOnLogin() {
  ...
  try {
    ...
    if (data.success && data.expenses && data.expenses.length > 0) {
      ...
      console.log(`Login sync: ${importedCount} new, ${updatedCount} updated`);
...
// ==================== CSV EXPORT/IMPORT ====================
// FIXED: Export to CSV function - now properly exports user's entries
...
// Updated CSV import with batch ID generation
async function importFromCSV(event) {
  ...
  reader.onload = async function(e) {
    ...
    const generatedIds = await startCSVImportIdGeneration(rowsToImport.length);
    ...
    try {
      ...
      recentEntries.forEach((e, i) => {
        ...
        row.innerHTML = `
          ...
          <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px; text-align: right; font-weight: 700; font-size: 1.05em;" class="${e.amount < 0 ? 'amount-negative' : 'amount-positive'}">
    ...
    try {
      ...
      filtered.forEach((e, i) => {
        ...
        row.innerHTML = `
          ...
          <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px;">${e.main}</td>
          <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px;">${e.sub}</td>
          <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px; text-align: right; font-weight: 700; font-size: 1.05em;" class="${e.amount >= 0 ? 'amount-positive' : 'amount-negative'}">
            ${e.amount >= 0 ? "+" : ""}PKR ${Math.abs(e.amount).toFixed(2)}
          </td>
          <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 110px; text-align: center;">
            <button onclick="editSavedEntryFromOtherPage('${e.id}')" class="small-btn edit-btn">✏️</button>
  ...
  try {
    ...
    results.forEach((entry, i) => {
      ...
      row.innerHTML = `
        ...
        <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 130px; text-align: right; font-weight: 700; font-size: 1.05em;" class="${entry.amount >= 0 ? 'amount-positive' : 'amount-negative'}">
          ${entry.amount >= 0 ? "+" : ""}PKR ${Math.abs(entry.amount).toFixed(2)}
        </td>
        <td style="padding: 12px; border-bottom: 1px solid #eaeaea; min-width: 110px; text-align: center;">
...
function getUserEntries(callback) {
  getAllEntries(entries => {
    const userEntries = entries.filter(e => e.userId === currentUserId || !e.userId);
...
// Mark entries as synced for current user
async function markAsSynced(entryIds) {
  return new Promise((resolve, reject) => {
    getUserEntries(async (entries) => {
      let updatedCount = 0;
...
console.log("✅ app.js COMPLETE ORIGINAL WITH ALL FIXES APPLIED loaded with Base61 IDs and user isolation");
