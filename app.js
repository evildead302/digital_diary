// app.js - COMPLETE FIXED VERSION - ALL ISSUES RESOLVED
...
function generateEntryId() {
  ...
  // FIXED: Use Base61.generateEntryId with currentUserId
...
async function syncWithCloud() {
  ...
  try {
    ...
    const expensesToSync = userUnsyncedEntries.map(entry => ({
      id: entry.id,
      date: entry.date,
    ...
    if (data.success) {
      ...
      alert(`✅ Sync TO Cloud Complete!\n• ${data.successes?.length || 0} entries synced`);
...
async function getEntriesNeedingSync() {
  return new Promise((resolve, reject) => {
    ...
    getAllEntries(entries => {
      // FIXED: Only get entries for current user that need sync
      const userEntries = entries.filter(e => e.userId === currentUserId);
      const needsSync = userEntries.filter(e => !e.synced || e.syncRemarks !== "synced");
      resolve(needsSync);
    });
  });
}
...
async function loadDataFromCloudOnLogin() {
  ...
  try {
    ...
    if (data.success && data.expenses && data.expenses.length > 0) {
      ...
      console.log(`Login sync: ${importedCount} new`);
...
function getUserEntries(callback) {
  getAllEntries(entries => {
    // FIXED: Only show entries for current user
    const userEntries = entries.filter(e => e.userId === currentUserId);
...
console.log("✅ app.js COMPLETE FIXED loaded with all issues resolved");
