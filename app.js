...
async function initUserDatabase() {
    ...
    // FIX: currentUserId should be declared here or passed as parameter
    if (!currentUserId) {
    ...
    try {
        ...
        if (typeof window.dbAPI?.initDatabaseForUser === 'function') {
            ...
            if (typeof window.addDebugLog === 'function') {
                window.addDebugLog('app.js: Using legacy database setup', 'warning');
            }
            return true;
        } else {
...
async function checkAuth() {
    ...
    let currentUserId = null; // DECLARE HERE
