// app.js - COMPLETE VERSION WITH USER ISOLATION AND FIXED TABLES
...
// Format amount for display with PKR and commas
function formatAmount(amount) {
    const sign = amount >= 0 ? '+' : '-';
    const absAmount = Math.abs(amount);
    const formattedNumber = absAmount.toLocaleString('en-PK', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
    });
...
// Format amount without sign for totals
...
function logout() {
    if (confirm("Are you sure you want to logout?")) {
        ...
        // Clear the current user's IndexedDB database
        if (currentUserId) {
            const dbName = `AccountsDiaryDB_${currentUserId}`;
            console.log(`Deleting current user database on logout: ${dbName}`);
            ...
            try {
                ...
                deleteRequest.onblocked = () => {
                    console.warn(`Database ${dbName} is blocked. Trying again...`);
                    setTimeout(() => {
        ...
        // Also clear any heads data from localStorage
        if (currentUserId) {
            const userHeadsKey = `heads_${currentUserId}`;
            if (localStorage.getItem(userHeadsKey)) {
                localStorage.removeItem(userHeadsKey);
                console.log(`Removed heads data for user: ${currentUserId}`);
            }
        }
        ...
        setTimeout(() => {
            console.log("Reloading page for clean logout...");
...
function clearOtherUserData() {
    ...
    // Clear previous user's IndexedDB data - MORE AGGRESSIVE APPROACH
    clearAllOtherUsersDatabases(userId);
...
// More aggressive function to clear all other users' databases
async function clearAllOtherUsersDatabases(currentUserId) {
    try {
        console.log("🔍 Looking for IndexedDB databases to clear...");
        
        // Get all databases that might exist
        const allDatabases = await getAllPossibleDatabases();
        
        // Filter out current user's database
        const currentUserDB = `AccountsDiaryDB_${currentUserId}`;
        const otherUserDBs = allDatabases.filter(dbName => 
            dbName !== currentUserDB
        );
        
        console.log(`Found ${otherUserDBs.length} other databases to clear:`, otherUserDBs);
        
        // Try to close any open database connection first
        if (window.dbAPI?.closeDatabase) {
            window.dbAPI.closeDatabase();
        }
        
        // Clear localStorage tracking of other users
        clearOtherUsersFromLocalStorage(currentUserId);
        
        // Try to delete each database
        for (const dbName of otherUserDBs) {
            try {
                const result = await tryDeleteDatabase(dbName);
                if (result) {
        ...
        // Also clear any residual localStorage entries
        clearResidualLocalStorage(currentUserId);
        
        console.log("✅ Other users' databases cleared");
        
    } catch (error) {
        console.error("❌ Error clearing other users' databases:", error);
    }
}

// Try multiple methods to get database names
async function getAllPossibleDatabases() {
    const databases = [];
    ...
    try {
        if (window.indexedDB && window.indexedDB.databases) {
            ...
            dbList.forEach(db => {
                if (db.name && db.name.startsWith('AccountsDiaryDB_')) {
                    databases.push(db.name);
                }
            });
        }
    } catch (error) {
        console.log("Method 1 (indexedDB.databases) failed:", error.message);
    }
    
    // Method 2: Check localStorage for known user IDs
    try {
        // Get all user IDs from localStorage
        const allKeys = Object.keys(localStorage);
        const potentialUserIds = [];
        
        // Look for auth_user entries
        allKeys.forEach(key => {
            if (key === 'auth_user') {
                try {
                    const userData = JSON.parse(localStorage.getItem(key));
                    if (userData && userData.id) {
                        potentialUserIds.push(userData.id);
                    }
                } catch (e) {
        ...
        // Create database names for these user IDs
        potentialUserIds.forEach(userId => {
        ...
        console.log("Method 2 (localStorage scan) failed:", error.message);
    }
    
    // Method 3: Try common database names
    try {
        // Try to open and close databases to see which ones exist
        const commonSuffixes = ['pUqxbButo4', 'pUqxWta4Kz'];
        commonSuffixes.forEach(userId => {
            const dbName = `AccountsDiaryDB_${userId}`;
            if (!databases.includes(dbName)) {
                databases.push(dbName);
            }
        ...
        console.log("Method 3 (common names) failed:", error.message);
    }
    
    return [...new Set(databases)];
}

// Try to delete a database with retry logic
function tryDeleteDatabase(dbName) {
    return new Promise((resolve, reject) => {
        ...
        const attemptDelete = () => {
            ...
            try {
                ...
                deleteRequest.onerror = (error) => {
                    console.error(`Error deleting database ${dbName} (attempt ${attempts}):`, error);
                ...
                deleteRequest.onblocked = () => {
                    console.warn(`Database ${dbName} is blocked. Trying to close connections...`);
                    ...
                    setTimeout(() => {
                        if (attempts < maxAttempts) {
                            ...
                            console.error(`Database ${dbName} could not be deleted due to being blocked`);
                            resolve(false);
                        }
                    }, 1000);
                };
            } catch (error) {
                console.error(`Exception deleting database ${dbName}:`, error);
                resolve(false);
            }
        };
...
// Clear other users from localStorage
function clearOtherUsersFromLocalStorage(currentUserId) {
    const allKeys = Object.keys(localStorage);
    
    allKeys.forEach(key => {
        if (key.startsWith('heads_') || key.includes('_sync') || key.includes('_last')) {
            ...
            patterns.forEach(userId => {
                if (userId !== currentUserId && key.includes(userId)) {
...
// Clear residual localStorage entries
function clearResidualLocalStorage(currentUserId) {
    const residualKeys = [
        'sync_timestamp',
        'last_sync',
        'sync_errors',
    ...
    residualKeys.forEach(key => {
        if (localStorage.getItem(key)) {
            console.log(`Clearing residual key: ${key}`);
            localStorage.removeItem(key);
        }
    });
}
    ...
    try {
        ...
        tempEntries.forEach((entry, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${entry.main}</td>
                <td>${entry.sub}</td>
                <td style="word-wrap: break-word; white-space: normal;">${entry.desc}</td>
                <td class="${entry.amount < 0 ? 'amount-negative' : 'amount-positive'}">
                    ${entry.amount < 0 ? "-" : "+"}PKR ${formatAmountPlain(Math.abs(entry.amount))}
                </td>
...
function exportToCSV() {
    getUserEntries(entries => {
        ...
        activeEntries.forEach(e => {
            ...
            // Export date in YYYY-MM-DD format for Excel compatibility
            ...
            csv += `"${e.id}","${formattedDate}","${e.desc}","${e.main}","${e.sub}",${e.amount},"${type}","${syncStatus}"\n`;
        });
...
async function importFromCSV(event) {
    ...
    reader.onload = async function(e) {
        ...
        const rowsToImport = parseCSVLines(lines);
        ...
        await processImportRows(rowsToImport);
        event.target.value = '';
    };
...
function showPasteCSVDialog() {
    ...
    modalContent.innerHTML = `
        <h3 style="margin-top: 0;">📋 Paste CSV Data</h3>
        <p style="color: #666; font-size: 0.9em; margin-bottom: 15px;">
            Paste comma-separated values. Each line is a transaction.<br>
            <strong>Format:</strong> Date, Description, Main Category, Sub Category, Amount<br>
            <em>Example:</em> 2024-01-15, Grocery shopping, Food, Groceries, 1500.00
        </p>
        <textarea id="csvPasteData" rows="8" style="
            width: 100%;
            padding: 10px;
            ...
            font-size: 14px;
            resize: vertical;
        " placeholder="2024-01-15, Grocery shopping, Food, Groceries, 1500.00
2024-01-16, Salary received, Income, Salary, 50000.00
2024-01-17, Coffee shop, Food, Dining, 250.00"></textarea>
        <div style="margin-top: 15px; display: flex; gap: 10px; justify-content: flex-end;">
            <button onclick="closePasteCSVDialog()" class="secondary-btn" style="padding: 10px 20px;">Cancel</button>
            <button onclick="processPastedCSV()" class="primary-btn" style="padding: 10px 20px;">Import Data</button>
        </div>
        <div id="csvPasteStatus" style="margin-top: 10px; font-size: 0.9em;"></div>
    `;
...
function closePasteCSVDialog() {
    const modal = document.getElementById('csvPasteModal');
    if (modal) modal.remove();
}

async function processPastedCSV() {
    const textarea = document.getElementById('csvPasteData');
    const statusDiv = document.getElementById('csvPasteStatus');
    
    if (!textarea || !textarea.value.trim()) {
        if (statusDiv) {
            statusDiv.innerHTML = '<span style="color: #f44336;">❌ Please paste some data first.</span>';
        } else {
    ...
    if (rowsToImport.length === 0) {
        if (statusDiv) {
            statusDiv.innerHTML = '<span style="color: #f44336;">❌ No valid rows found. Please check the format.</span>';
        } else {
            alert('No valid rows found. Please check the format.');
        }
        return;
    }
    
    if (statusDiv) {
        statusDiv.innerHTML = `<span style="color: #2196f3;">🔄 Importing ${rowsToImport.length} entries...</span>`;
    }
...
function parseCSVLines(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (cleanParts.length >= 5) {
            try {
                let amount = parseFloat(cleanParts[4]);
                if (!isNaN(amount)) {
                    let dateValue = cleanParts[0];
                    ...
                    console.warn(`Invalid amount at line ${i+1}: ${cleanParts[4]}`);
                }
            } catch (error) {
                console.error(`Error parsing row ${i+1}:`, error);
            }
        } else if (cleanParts.length === 5 && cleanParts[0].toLowerCase() === 'date') {
            continue;
        } else {
            console.warn(`Skipping line ${i+1}: expected 5 columns, got ${cleanParts.length}`);
        }
    }
...
async function processImportRows(rowsToImport) {
    let imported = 0;
    let errors = 0;
    ...
    for (let i = 0; i < rowsToImport.length; i++) {
        try {
            ...
            if (entry.main) {
                newCategories.add(entry.main);
                if (!heads[entry.main]) {
                    heads[entry.main] = [];
                }
                if (entry.sub && !heads[entry.main].includes(entry.sub)) {
                    heads[entry.main].push(entry.sub);
                }
            }
        } catch (error) {
            console.error("❌ Error saving imported row:", error);
            errors++;
        }
    }
...
function setupEntryScroll() {
    ...
    if (tempList) {
        tempList.style.overflow = "visible";
    }
    
    if (savedList) {
        savedList.style.overflow = "visible";
    }
}
        ...
        if (page) {
            ...
            switch(id) {
                ...
                case "search":
                    ...
                    // Clear previous search results when opening search page
                    setTimeout(() => {
                        const searchResults = document.getElementById("searchResults");
                        if (searchResults) searchResults.innerHTML = "";
                        const searchCount = document.getElementById("searchResultsCount");
                        if (searchCount) searchCount.textContent = "🔍 Found 0 transactions";
                        const searchTotal = document.getElementById("searchTotal");
                        if (searchTotal) searchTotal.textContent = "💰 Total: PKR 0.00";
            ...
            recentEntries.forEach((e, i) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="${e.amount < 0 ? 'amount-negative' : 'amount-positive'}">
                        ${e.amount < 0 ? "-" : "+"}PKR ${formatAmountPlain(Math.abs(e.amount))}
                    </td>
                    <td style="text-align: center;">
                        ${syncIcon}
                    </td>
            ...
            getUserEntries(async d => {
                try {
                    ...
                    const editedEntry = {
                        ...
                        synced: false,  // Force re-sync to cloud
                        syncRemarks: "edited",
                    ...
                    alert("✅ Entry updated successfully! Will sync to cloud on next sync.");
                } catch (error) {
...
function calcTotal() {
    getUserEntries(d => {
        try {
            ...
            if (totalBalance) {
                totalBalance.innerText = `PKR ${formatAmountPlain(total)}`;
            ...
            if (totalIncome) totalIncome.innerText = `PKR ${formatAmountPlain(income)}`;
            if (totalExpense) totalExpense.innerText = `PKR ${formatAmountPlain(expense)}`;
            if (netBalance) {
                netBalance.innerText = `PKR ${formatAmountPlain(total)}`;
            ...
            updateSyncStatus(activeEntries);
            ...
            li.innerHTML = `
                <div class="main-category-balance">
                    ...
                    <span class="main-cat-total ${mainTotal >= 0 ? "positive" : "negative"}">
                        PKR ${formatAmountPlain(Math.abs(mainTotal))}
                    </span>
                </div>
                    ...
                    ${Object.entries(map[main]).map(([sub, amount]) => `
                        <li>
                            ...
                            <span class="sub-cat-amount ${amount >= 0 ? "positive" : "negative"}">
                                ${amount >= 0 ? "+" : "-"}PKR ${formatAmountPlain(Math.abs(amount))}
                            </span>
                        </li>
                    `).join("")}
                </ul>
...
function loadLedger() {
    getUserEntries(d => {
        try {
            ...
            // FIXED: Sort by date correctly - newest first using proper date parsing
            filtered.sort((a, b) => {
                return parseDateString(b.date) - parseDateString(a.date);
            });
            ...
            if (statsElement) {
                statsElement.innerHTML = `
                    ...
                    Total: <span style="color:${total >= 0 ? "#2e7d32" : "#c62828"}">PKR ${formatAmountPlain(Math.abs(total))}</span> |
                    Income: <span style="color:#2e7d32">PKR ${formatAmountPlain(incomeTotal)}</span> |
                    Expense: <span style="color:#c62828">PKR ${formatAmountPlain(expenseTotal)}</span>
                `;
            }
            ...
            filtered.forEach((e, i) => {
                ...
                row.innerHTML = `
                    ...
                    <td>${e.main}</td>
                    <td>${e.sub}</td>
                    <td style="word-wrap: break-word; white-space: normal; min-width: 250px; max-width: 400px;">${e.desc}</td>
...
function displaySearchResults(results, searchTerm = "") {
    try {
        ...
        if (totalElement) {
            totalElement.textContent = `💰 Total: PKR ${formatAmountPlain(Math.abs(totalAmount))}`;
...
window.showPasteCSVDialog = showPasteCSVDialog;
window.closePasteCSVDialog = closePasteCSVDialog;
window.processPastedCSV = processPastedCSV;
