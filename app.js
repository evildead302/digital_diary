...
async function loadDataFromCloudOnLogin() {
    ...
    try {
        ...
        if (data.success && data.expenses && data.expenses.length > 0) {
            ...
            console.log(`Imported ${importedCount} new expenses, updated ${updatedCount} from cloud`);
            
            if (importedCount > 0 || updatedCount > 0) {
                alert(`✅ Loaded ${importedCount} new entries, updated ${updatedCount} from cloud`);
            }
...
function exportToCSV() {
    getAllEntries(entries => {
        ...
        let csv = "Date,Description,Main Category,Sub Category,Amount,Type,Sync Status\n";
        activeEntries.forEach(e => {
            const type = e.amount >= 0 ? "Income" : "Expense";
            const syncStatus = e.synced ? (e.syncRemarks === "edited" ? "Edited" : "Synced") : "Pending";
            csv += `"${e.date}","${e.desc}","${e.main}","${e.sub}",${e.amount},"${type}","${syncStatus}"\n`;
        });
...
async function syncWithCloud() {
    ...
    const syncBtn = document.getElementById("syncBtn") || document.querySelector('button[onclick="syncWithCloud()"]');
    ...
    try {
        ...
        if (data.success) {
            // Mark entries as synced in IndexedDB
            const entryIds = unsyncedEntries.map(e => e.id);
            const result = await markAsSynced(entryIds);
            
            // Also load any new data from cloud (two-way sync)
            await loadDataFromCloudOnLogin();
        ...
        alert(`❌ Sync failed: ${error.message}. Please check your API connection.`);
    } finally {
        if (syncBtn) {
            syncBtn.textContent = originalText;
            syncBtn.disabled = false;
...
// ==================== ADD CSV BUTTONS TO SETTINGS ====================
function addCSVButtonsToSettings() {
    const settingsSection = document.querySelector("#settings .settings-section:last-child");
    if (settingsSection && !document.getElementById("csvButtons")) {
        const csvSection = document.createElement("div");
        csvSection.id = "csvButtons";
        csvSection.className = "settings-section";
        csvSection.innerHTML = `
            <h3>4️⃣ CSV Import/Export</h3>
            <div class="button-group">
                <button onclick="exportToCSV()" class="primary-btn">📥 Export All to CSV</button>
                <label class="secondary-btn" style="cursor: pointer; display: inline-block;">
                    📤 Import from CSV
                    <input type="file" id="csvImportInput" accept=".csv" 
                           style="display: none;" onchange="importFromCSV(event)">
                </label>
            </div>
            <p style="margin-top: 10px; color: #666; font-size: 0.9em;">
                Note: CSV import will add new entries. Existing entries with same ID will be updated.
            </p>
        `;
        settingsSection.parentNode.insertBefore(csvSection, settingsSection.nextSibling);
    }
}
...
window.onload = function() {
    try {
        ...
        // Add CSV buttons to settings
        setTimeout(addCSVButtonsToSettings, 1000);
...
document.addEventListener("DOMContentLoaded", function() {
    // Add sync button to home if not exists
    const quickActions = document.querySelector(".quick-actions");
    if (quickActions && !quickActions.querySelector("#syncBtn")) {
        const syncBtn = document.createElement("button");
        syncBtn.id = "syncBtn";
        syncBtn.className = "success-btn";
        syncBtn.textContent = "🔄 Sync with Cloud";
        syncBtn.onclick = syncWithCloud;
...
// (These are from your original app.js - they should be included as-is)
