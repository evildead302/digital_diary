...
// ==================== FIXED SYNC WITH CLOUD (SYNC TO CLOUD) ====================
async function syncWithCloud() {
    ...
    try {
        // Get unsynced entries including deleted ones
        ...
        if (otherEntries.length > 0) {
            ...
            if (data.success) {
                ...
                if (data.errors && data.errors.length > 0) {
                    ...
                    // All entries successful - mark all as synced
                    const entryIds = otherEntries.map(e => e.id);
        ...
        // Handle deleted entries - send DELETE request to server
        let deletedSyncSuccess = true;
        let deletedErrors = [];
        ...
        if (deletedEntries.length > 0) {
            ...
            for (const deletedEntry of deletedEntries) {
                try {
                    const deleteResponse = await fetch(`/api/expenses/${deletedEntry.id}`, {
                        method: "DELETE",
                        headers: {
                            "Authorization": `Bearer ${authToken}`
                        }
        ...
        if (totalErrors === 0) {
            ...
            errorMessage += `\n❌ ${totalErrors} errors occurred. Failed entries were NOT marked as synced and will retry on next sync.`;
        ...
        // CRITICAL: DO NOT mark entries as synced if there was an error
        alert(`❌ Sync TO Cloud failed completely: ${error.message}\n\nNo entries were marked as synced. All pending entries will retry on next sync.`);
    } finally {
        if (syncBtn) {
            syncBtn.textContent = originalText;
            syncBtn.disabled = false;
    ...
    try {
        ...
        if (page) {
            ...
            switch(id) {
                case "home":
                    loadHomeData();
                    updateSyncButtons(); // Ensure sync buttons are properly displayed
                    break;
                case "entry":
...
function addCSVButtonsToSettings() {
    ...
    if (settingsContainer && !document.getElementById("csvButtons")) {
        // Find existing settings sections
        const existingSections = settingsContainer.querySelectorAll('.settings-section');
        
        const csvSection = document.createElement("div");
        csvSection.id = "csvButtons";
        csvSection.className = "settings-section csv-section";
        ...
        // Insert after the last settings section
        if (existingSections.length > 0) {
            existingSections[existingSections.length - 1].parentNode.insertBefore(csvSection, existingSections[existingSections.length - 1].nextSibling);
        } else {
            settingsContainer.appendChild(csvSection);
        }
    }
}
